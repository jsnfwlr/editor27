 ...
 */
abstract class editor_tinymcefour_plugin {
    ...
    protected function load_config() {
        if (!isset($this->config)) {
            $name = $this->get_name();
            $this->config = get_config("tinymcefour_$name");
        }
    }
    ...
    public function set_config($name, $value) {
        ...
        set_config($name, $value, "tinymcefour_$pluginname");
    }
    ...
    public function get_name() {
        // All class names start with "tinymcefour_".
     ...
     */
    protected function add_button_after(array &$params, $button,
        ...
        if ($alwaysadd) {
            return $this->add_button_after($params, $button);
        }
     ...
     * @return bool True if added or button already exists (in any row)
     */
    protected function add_button_before(array &$params, $button,
        ...
        // If always adding, recurse to add it empty.
        if ($alwaysadd) {
            return $this->add_button_before($params, $button);
        }
    ...
    protected function find_button(array &$params, $button) {
        return strpos(' ' . $params['toolbar'] . ' ', ' ' . $button . ' ') !== false;
    }
     ...
     */
    public function get_tinymcefour_file_url($file='', $absolute=true) {
        global $CFG;
        ...
        if ($CFG->slasharguments) {
            ...
            $jsurl = 'loader.php/' . '/' . $version . '/plugins/' . $this->plugin . '/tinymce/' . $file;
        } else {
    ...
    public static function all_update_init_params(array &$params,
        ...
        $plugins = core_component::get_plugin_list('tinymcefour');
        ...
        foreach ($plugins as $plugin => $dir) {
            ...
            $classname = 'tinymcefour_' . $plugin;
            $pluginobjects[] = new $classname($plugin);
        }

        // Sort plugins by sort order and name.
        usort($pluginobjects, array('editor_tinymcefour_plugin', 'compare_plugins'));
     ...
     * @return editor_tinymcefour_plugin Plugin object
     */
    public static function get($plugin) {
        $dir = core_component::get_component_directory('tinymcefour_' . $plugin);
        require_once($dir . '/lib.php');
        $classname = 'tinymcefour_' . $plugin;
     ...
     * @param editor_tinymcefour_plugin $a
     * @param editor_tinymcefour_plugin $b
     * @return Negative number if $a is before $b
     */
    public static function compare_plugins(editor_tinymcefour_plugin $a, editor_tinymcefour_plugin $b) {
